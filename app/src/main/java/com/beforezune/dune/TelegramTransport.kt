...
class TelegramTransport(private val config: TelegramConfigStore) : BackupTransport {
    override suspend fun upload(file: File): Result<String> {
        val token = config.botToken()
            ?: return Result.failure(IllegalStateException("Telegram backup is not configured"))
        ...
        return try {
            ...
            if (code !in 200..299) throw IllegalStateException("Telegram upload failed: HTTP $code")
            connection.disconnect()
            Result.success("uploaded:${file.name}")
